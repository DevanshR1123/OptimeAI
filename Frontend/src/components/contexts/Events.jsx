...
export const EventsProvider = ({ children }) => {
    ...
    const processEvents = async (timetable) => {
        ...
        const { items } = await getEvents(
            nextMonday,
            nextMonday + 7 * 24 * 60 * 60 * 1000,
            false,
        );
        const calendarEvents = items.map((item) => item.id);
        const existingEvents = timetable.filter((event) =>
            calendarEvents.includes(event.gEventId),
        );
    ...
    const addEvent = (tileId, from, duration, day) => {
        ...
        events.forEach((event) => {
            ...
            const exactMatch =
                from === event.from && duration === event.duration;
            ...
            if (dayMatch && intersectMatch && !tileMatch && exactMatch) {
                ...
                const existingLog = changeLog.find(
                    (change) => change.data.id === event.id,
                );
                if (exactMatch) {
                    deleteEvent(event.id);
                ...
                if (!exactMatch) {
                    ...
                    if (event.gEventId && !existingLog)
                        setChangeLog((changeLog) => [
                            ...changeLog,
                            { type: "modify", data: { id: event.id } },
                        ]);
                }
            }
        });
    ...
    const deleteEvent = (id) => {
        ...
        setChangeLog((changeLog) =>
            changeLog.filter((change) => change.data.id !== id),
        );
        if (event.gEventId)
            setChangeLog((changeLog) => [
                ...changeLog,
                { type: "delete", data: { gEventId: event.gEventId } },
            ]);
    };
    ...
    const processChangeLog = async () => {
        ...
        for (const log of changeLog) {
            const { type, data } = log;
            if (type === "add") {
                const { id, tileId, from, duration, day } = events.find(
                ...
                const gEvent = await createWeeklyEvent(
                    summary,
                    fromDateTime,
                    toDateTime,
                    description,
                ...
                const { id } = data;
                const { gEventId, tileId, from, duration, day } = events.find(
                    (event) => event.id === id,
                );
                const fromDateTime = getDate(day, from);
                const toDateTime = getDate(day, from + duration);
    ...
    const handleTimetableSave = async () => {
        try {
            ...
            const finalEvents = events.map((event) => {
                const gEventId = processedEvents.find(
                    (gEvent) => gEvent.id === event.id,
                )?.gEventId;
